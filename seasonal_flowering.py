...
def evolution_step(resident_surface, mutation_function, atol, mutation_epsilon, domain, fitness_function, alpha, beta, k_function):
    """
    ...
    [fitness_resident, fitness_mutant] = fitness_function(
        resident_surface, mutant, domain, alpha, beta, k_function)
...
def evolve(initial_surface, mutation_function, iterations, mutation_epsilon, domain, fitness_function, alpha, beta, k_function):
    """
    Evolve to a target
    ...
    for step in xrange(1, iterations):
        ...
        resident, invasion = evolution_step(
            resident, mutation_function, atol, mutation_epsilon, domain, fitness_function, alpha, beta, k_function)
...
def create_video_from_time_series(series_compact, target_surface, domain, filename, approximate_number_of_frames, record_every):
    ...
    if target_surface is not None:
        y_limit = (min(target_surface) - 0.01, max(target_surface) + 0.01)
        x_limit = (min(domain) - 0.01, max(domain) + 0.01)
